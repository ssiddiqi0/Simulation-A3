...
struct Node_s {
    ...
    int next; // index for next node in array
    int prev; // index for previous node in array
    void* data; // pointer to the item being held by the node
    bool free;
};
...
struct List_s{
    ...
    int count; // keeps track of elements in array 
    int next; // next list if there is one
